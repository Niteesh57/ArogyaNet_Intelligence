...
export const AppointmentChatModal = ({ open, onClose, appointmentId, patientName }: AppointmentChatModalProps) => {
    ...
    const handleSend = async () => {
        ...
        // Optimistic update
        const tempMsg: ChatMessage = { message: question, user_id: user?.id, created_at: new Date().toISOString() };
        ...
        try {
            ...
            setMessages(prev => prev.map((m, i) =>
                i === prev.length - 1 ? { ...m, response: res.data.answer, document_url: currentDocUrl, user_id: user?.id } : m
            ));
